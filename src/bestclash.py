...
# ---------------- Convert to one-line YAML mapping lines ----------------
def proxies_to_one_line_yaml(proxies):
    """
    Build list of "- { name: '...', type: vmess, server: x, port: n, ... }" lines.
    Map protocol-specific fields to Clash-friendly keys.
    """
    lines = []
    for p in proxies:
        entry = {}
        entry["name"] = p.get("name") or p.get("ps") or p.get("server")
        ...
        if proto in ("vmess",):
            ...
            # Clash expects alterId sometimes as int
            if p.get("alterId") is not None:
                try:
                    entry["alterId"] = int(p.get("alterId"))
                except Exception:
            ...
            # copy TLS/SNI/path if present in raw payload or params
            ...
            if p.get("port"):
                entry["port"] = int(p.get("port"))
            # map method -> cipher to match your example
            if p.get("method"):
                entry["cipher"] = p.get("method")
            if p.get("password"):
                entry["password"] = p.get("password")
            # udp flag
            if p.get("udp") in (True, "true", "True", 1):
                entry["udp"] = True
        elif proto in ("ssr",):
            entry["type"] = "ssr"
            entry["server"] = p.get("server")
        ...
        elif proto in ("hysteria", "hysteria2", "anytls", "anytls2", "tuic"):
            # Hysteria/anytls/TUIC are preserved as their scheme name; ensure server/port exist
            entry["type"] = proto
            entry["server"] = p.get("server")
            if p.get("port"):
                entry["port"] = int(p.get("port")) if p.get("port") else 0
            # include token/auth if present
            if p.get("password"):
                entry["password"] = p.get("password")
            if isinstance(p.get("params"), dict):
                for k, v in p.get("params").items():
            ...
            # generic fallback - keep type as original or "generic"
